...
class EventsController extends AppController
{
    ...
    public function isAuthorized($user = null)
    {
        ...
        if (in_array($this->request->action, ['cancel', 'edit'])) {
    ...
    public function calendar($year = null, $month = null, $day = null)
    {
        $this->Crud->on('beforePaginate', function(\Cake\Event\Event $event) {
            ...
            // RIP Miranda Coburn and Jonas Hunter
        ...
        if (isset($_GET['start_date']) && isset($_GET['end_date'])) {
            ...
            $data = $this->Events->find('all')
                ->contain([
                    ...
                    'Registrations' => function ($q) {
                        return $q->where(['Registrations.status' => 'confirmed']);
                    }
                ])
                ->where([
    ...
    public function _beforeCreate(\Cake\Event\Event $event)
    {
        ...
        // Check if address data is set and flag w9 as on file and vendor_id after communication with quickbooks
		$propagationStopped = false;
		if (!empty($event->subject()->entity->contact['taxid'])) {
			$namePieces = explode(' ', $event->subject()->entity->contact['name'], 2);
			
			$result = $this->__syncQuickBooksVendor(array(
				'BillAddr' => array(
					'Line1' => $event->subject()->entity->contact['address1'],
					'Line2' => $event->subject()->entity->contact['address2'],
					'City' => $event->subject()->entity->contact['city'],
					'Country' => 'U.S.A.',
					'CountrySubDivisionCode' => $event->subject()->entity->contact['state'],
					'PostalCode' => $event->subject()->entity->contact['zip'],
				),
				'TaxIdentifier' => $event->subject()->entity->contact['taxid'],
				'GivenName' => $namePieces[0],
				'FamilyName' => $namePieces[1],
				'PrimaryPhone' => array(
					'FreeFormNumber' => $event->subject()->entity->contact['phone']
				),
				'PrimaryEmailAddr' => array(
			    	'Address' => $event->subject()->entity->contact['email']
				)
			));
			
			if ($result['success']) {
				$event->subject->entity->contact->w9['vendor_id'] = $result['message'];
				$event->subject()->entity->contact->w9_on_file = true;
			} else {
				$propagationStopped = true;
				$event->subject()->entity->honorarium->errors('pay_contact', [$result['message']]);
        ...
        if ($event->subject()->entity->request_honorarium && $event->subject()->entity->honorarium->pay_contact && !$event->subject()->entity->contact->w9_on_file) {
			if (!$propagationStopped) {
				$event->subject()->entity->honorarium->errors('pay_contact', ['Tax information is required to be on file for honorarium.']);
			}
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($mode == 'add') {
                ...
                if (isset($this->request->data['contact_id'])) {
                    ...
                    $contact = $this->Contacts->get($this->Auth->user('contact_id'));
                ...
                /**
                 * Unset w9 fields when not used. If file fields are present and empty
                 * the submission will fail validation.
                 */
				if (isset($this->request->data['contact']['taxid']) && empty($this->request->data['contact']['taxid'])) {
					unset($this->request->data['contact']['address1']);
					unset($this->request->data['contact']['address2']);
					unset($this->request->data['contact']['city']);
					unset($this->request->data['contact']['state']);
					unset($this->request->data['contact']['zip']);
					unset($this->request->data['contact']['taxid']);
				}
	...
	private function __syncQuickBooksVendor($vendor)
	{	
		$requestUrl = 'https://quickbooks.api.intuit.com/v3/company/1410527430/vendor';
		
		$oauth = new \OAuth(
			'qyprde47awWUc0NeOtxhWx7Jj5q6IK', // Consumer key
			'xatcSV9cP0OPF4808WzLI1xUKNgBTZ8lBZvKRHhz' // Consumer secret
		);
		$oauth->setToken(
			'lvprdcjOV77nVejs4uiErpESpzXUt3PUsK2eLEpnoEQ2kJDA', // Token
			'yTAUaP1bcLwcGXF0SGr7teem8X7cvZqVnGT9PoLI' // Token secret
		);
		$authHeader = $oauth->getRequestHeader('POST', $requestUrl);
		...
		curl_setopt_array($curl, array(
			...
			CURLOPT_POSTFIELDS => json_encode($vendor),
		...
		if ($err) {
			return array(
				'success' => false,
				'message' => 'There was an issue connecting to the tax information service. Try again later.'
			);
		} else {
			$response = json_decode($response);
			if (isset($response->Fault) && $response->Fault->Error[0]->code == 6240) {
				$vendorData = $this->__queryQuickbooksVendorID($vendor['GivenName'], $vendor['FamilyName']);
				if ($vendorData['success']) {
					return $vendorUpdate = $this->__updateQuickbooksVendor($vendorData['message'], $vendor);
				} else {
					return $vendorData;
				}
			} else if (isset($response->Vendor)) {
				return array(
					'success' => true,
					'message' => $response->Vendor->Id
				);
			} else {
				return array(
					'success' => false,
	...
	private function __queryQuickbooksVendorID($givenName, $familyName) {		
		$requestUrl = "https://quickbooks.api.intuit.com/v3/company/1410527430/query?query=select%20*%20from%20vendor%20where%20GivenName%20%3D%20'" . $givenName . "'%20AND%20FamilyName%20%3D%20'" . $familyName . "'";
		...
		$authHeader = $oauth->getRequestHeader('GET', $requestUrl);
		...
		if ($err) {
			...
			$response = json_decode($response);
			if (isset($response->QueryResponse->Vendor[0]->Id)) {
				return array(
					'success' => true,
					'message' => array(
						'id' => $response->QueryResponse->Vendor[0]->Id,
						'syncToken' => $response->QueryResponse->Vendor[0]->SyncToken
					)
				);
			} else {
				return array(
					'success' => false,
					'message' => 'Tax information was found, but failed to update automatically. Try again in a few minutes or contact finance to resolve the issue manually.'
				);
			}
		}
	}
	
	private function __updateQuickbooksVendor($vendorUpdate, $vendorData) {
		$vendorData['Id'] = $vendorUpdate['id'];
		$vendorData['SyncToken'] = $vendorUpdate['syncToken'];
		return $this->__syncQuickBooksVendor($vendorData);
	}
}
